...
function renderFeedSlide(item, index) {
  ...
  return `
    <div class="feed-slide bg-gray-900" data-item-id="${item.id}" data-index="${index}" data-url="${item.url}" data-platform="${item.platform}">
      ...
      <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black via-black/90 to-transparent p-6 pb-8">
        <!-- Category Badge -->
        <div class="mb-3">
          <span class="inline-block px-3 py-1 bg-blue-600/80 rounded-full text-xs font-medium">
            ${item.category}
          </span>
        </div>
        
        <!-- Product Name -->
        <h2 class="text-2xl font-bold mb-2">${item.productName}</h2>
        
        <!-- Description -->
        <p class="text-gray-300 text-sm mb-4 line-clamp-2">${item.productDescription}</p>
        
        <!-- Metrics Grid -->
        <div class="grid grid-cols-2 gap-3 mb-4">
          <div class="bg-gray-800/80 rounded-lg p-3">
            <div class="text-xs text-gray-400 mb-1">${TEXT_CONFIG.metric_views}</div>
            <div class="text-lg font-bold">${formatNumber(item.statsNow.views)}</div>
          </div>
          <div class="bg-gray-800/80 rounded-lg p-3">
            <div class="text-xs text-gray-400 mb-1">${TEXT_CONFIG.metric_likes}</div>
            <div class="text-lg font-bold">${formatNumber(item.statsNow.likes)}</div>
          </div>
          <div class="bg-gray-800/80 rounded-lg p-3">
            <div class="text-xs text-gray-400 mb-1">${TEXT_CONFIG.metric_comments}</div>
            <div class="text-lg font-bold">${formatNumber(item.statsNow.comments)}</div>
          </div>
          <div class="bg-gray-800/80 rounded-lg p-3">
            <div class="text-xs text-gray-400 mb-1">${TEXT_CONFIG.metric_saturation}</div>
            <div class="text-lg font-bold ${satColor}">${item.saturationScore}</div>
          </div>
        </div>
        ...
        <div class="mb-4">
          ...
          <span class="text-sm text-gray-400 ml-2">${TEXT_CONFIG.price_label}</span>
        </div>
        
        <!-- Action Button -->
        <button 
          id="save-btn-${item.id}"
          class="${item.saved ? 'px-6 py-2 bg-green-600 rounded-full font-medium' : 'px-6 py-2 bg-blue-600 rounded-full font-medium'}"
        >
